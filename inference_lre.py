...
from model.spaced_sampler_lre import SpacedSampler
...
def process(
    ...
    tile_size: int,
    tile_stride: int, 
    vq_imgs: Optional[List[np.ndarray]]=None
    ...
    control = control * 2 - 1
    if vq_imgs != None:
        vq = torch.tensor(np.stack(vq_imgs) / 255.0, dtype=torch.float32, device=model.device).clamp_(0, 1)
        vq = einops.rearrange(vq, "n h w c -> n c h w").contiguous()
        vq = vq * 2 - 1
    ...
    if cond_fn is not None:
        cond_fn.load_target(control)
    
    height, width = control.size(-2), control.size(-1)
    shape = (n_samples, 4, height // 8, width // 8)
    x_T = torch.randn(shape, device=model.device, dtype=torch.float32)
    # p_p = 'Clear, Cinematic, High Contrast, highly detailed, Color Grading, ultra HD, extreme meticulous detailing, hyper sharpness, perfect without deformations, high-resolution, 8k'
    # n_p = 'blurring, dirty, messy, worst quality, low quality, jpeg artifacts, lowres, over-smooth, dotted, noise'
    p_p = 'clean, high-resolution, 8k'
    n_p = 'dotted, noise, blur, lowres, smooth'
    # p_p = "Cinematic, High Contrast, highly detailed, taken using a Canon EOS R camera, hyper detailed photo - realistic maximum detail, 32k, Color Grading, ultra HD, extreme meticulous detailing, skin pore detailing, hyper sharpness, perfect without deformationsy"
    ...
    if not tiled:
        if vq_imgs != None:
            samples = sampler.sample(
                ...
                positive_prompt=p_p, negative_prompt=n_p, x_T=x_T,
                cfg_scale=cfg_scale, cond_fn=cond_fn,
            ...
            samples = sampler.sample(
                ...
                positive_prompt=p_p, negative_prompt=n_p, x_T=x_T,
                cfg_scale=cfg_scale, cond_fn=cond_fn,
                color_fix_type=color_fix_type, starts=starts
            )
    else:
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--starts", type=float, default=1.0)
...
def center_crop(pil_image, image_size):
...
def main() -> None:
    ...
    # load_state_dict(model.first_stage_model, torch.load('/my_project_path/DiffBIR-main/WGSR/experiments/VAE_1level_fail/models/100000_G.pth', map_location="cpu"), strict=False)
    ...
    for file_path in list_image_files(args.input, follow_links=True):
        ...
        if args.vq != None:
            vq = Image.open(file_path.replace(args.input, args.vq)).convert("RGB")
        # h, w = lq.size
        # lq = lq.resize((h // 4, w // 4), Image.BICUBIC)
        # lq = lq.resize((h, w), Image.BICUBIC)
        # lq = center_crop(lq, 128)
        if args.sr_scale != 1:
            lq = lq.resize(
        ...
        if args.vq != None:
            x_vq = pad(np.array(vq), scale=64)
            
        for i in range(args.repeat_times):
            save_path = os.path.join(args.output, os.path.relpath(file_path, args.input))
            parent_path, stem, _ = get_file_name_parts(save_path)
            if args.repeat_times > 1:
                parent_path = os.path.join(parent_path, str(i))
