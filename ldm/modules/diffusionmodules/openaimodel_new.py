    ...
    def forward(self, x, emb, context=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, HybridDecoupleTransformer):
                x = layer(x, context)
            else:
                x = layer(x)
        return x
...
class TResConnectBlock(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            nn.SiLU(),
            zero_module(conv_nd(dims, out_channels, self.out_channels, 3, padding=1)),
            nn.Sigmoid()
        )
    ...
    def _forward(self, x, emb):
        ...
        spatial_weight = self.in_layers(x)
        temporal_weight = self.emb_layers(emb)
        # emb_out = self.emb_layers(emb).type(h.dtype)
        # while len(emb_out.shape) < len(h.shape):
        #     emb_out = emb_out[..., None]
        temporal_weight = temporal_weight[..., None, None]

        h = temporal_weight * x0 + (1 - temporal_weight) * x1
        # print(th.mean(temporal_weight))
        return h


class SResConnectBlock(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
    ...
    def _forward(self, x, emb):
        ...
        h = spatial_weight * x0 + (1 - spatial_weight) * x1
        # print(th.mean(spatial_weight))
        return h


class ResBlockWoNorm(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
