...
def space_timesteps(num_timesteps, section_counts):
    ...
    for i, section_count in enumerate(section_counts):
        ...
        start_idx += size
    # all_steps = [0, 20, 41, 61, 82, 102, 122, 143, 163, 183, 204, 224, 245, 265, 285, 306, 326, 347, 367, 387, 408, 428, 449, 469, 489, 510, 530, 550, 571, 591, 612, 632, 652, 673, 693, 714, 734, 754, 775, 795, 816, 836, 856, 877, 897, 917, 938, 958, 979, 999]
    # all_steps = [510, 530, 550, 571, 591, 612, 632, 652, 673, 693, 714, 734, 754, 775, 795, 816, 836, 856, 877, 897, 917, 938, 958, 979, 999]
...
class SpacedSampler:
    ...
    def sample(
        ...
        cond = {
            ...
            # "c_emptytext": ([self.model.get_learned_conditioning([''] * b)] if not self.model.cond_stage_trainable else [self.model.get_learned_conditioning(dict(lq=[img_input], hq=[img_input], dec=[img_input], txt=[''] * b))])
        }
        uncond = {
            "c_latent": [self.model.apply_condition_encoder(img_input)],
        ...
        # apply color correction (borrowed from StableSR)
        if color_fix_type == "adain":
            img_pixel = adaptive_instance_normalization(img_pixel, (img_input + 1) / 2)
        elif color_fix_type == "wavelet":
            img_pixel = wavelet_reconstruction(img_pixel, (img_input + 1) / 2)
        else:
            assert color_fix_type == "none", f"unexpected color fix type: {color_fix_type}"
        return img_pixel
