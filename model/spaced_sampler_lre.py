...
def space_timesteps(num_timesteps, section_counts, starts=1.0):
    ...
    for i, section_count in enumerate(section_counts):
        ...
        if section_count <= 1:
            ...
            frac_stride = (size - 1 - start_idx) / (section_count - 1) * starts
        cur_idx = 0.0
        taken_steps = []
        ...
        start_idx += size
    # all_steps = [0, 20, 41, 61, 82, 102, 122, 143, 163, 183, 204, 224, 245, 265, 285, 306, 326, 347, 367, 387, 408, 428, 449, 469, 489, 510, 530, 550, 571, 591, 612, 632, 652, 673, 693, 714, 734, 754, 775, 795, 816, 836, 856, 877, 897]
...
class SpacedSampler:
    ...
    def make_schedule(self, num_steps: int, starts: float) -> None:
        """
        ...
        used_timesteps = space_timesteps(self.original_num_steps, str(num_steps), starts)
    ...
    def q_sample(
        ...
        # print(_extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape))
        # print(torch.mean(x_start))
        # print(torch.std(x_start))
        # print(_extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape))
    ...
    def predict_noise(
        ...
        if uncond is None or cfg_scale == 1.:
            ...
            if isinstance(model_cond, tuple):
                model_cond = model_cond[0]
                model_uncond = model_uncond[0]
    ...
    def sample(
        ...
        cond_fn: Optional[Guidance]=None,
        color_fix_type: str="none", 
        vq: Optional[torch.Tensor]=None
    ) -> torch.Tensor:
        self.make_schedule(num_steps=steps, starts=starts)
        ...
        img_input = cond_img if vq == None else vq
        cond = {
            "c_latent": [self.model.apply_condition_encoder(img_input)],
            "c_crossattn": ([self.model.get_learned_conditioning([positive_prompt] * b)] if not self.model.cond_stage_trainable else [self.model.get_learned_conditioning(dict(lq=[img_input], hq=[img_input], dec=[img_input], txt=[positive_prompt] * b))])
        }
        uncond = {
            "c_latent": [self.model.apply_condition_encoder(img_input)],
            "c_crossattn": ([self.model.get_learned_conditioning([negative_prompt] * b)] if not self.model.cond_stage_trainable else [self.model.get_learned_conditioning(dict(lq=[img_input], hq=[img_input], dec=[img_input], txt=[negative_prompt] * b), force_dropout=False)])
        }

        # img = cond['c_latent'][0]
        use_lre = True
        lre = cond['c_latent'][0].to(device)
        # lre = 0.1 * lre + 0.9 * torch.randn_like(lre)
        # lre = self.model.apply_condition_encoder(vq).to(device)
        # hre = self.model.apply_condition_encoder(cond['hq'][0].to(device)) / self.model.scale_factor

        for i, step in enumerate(iterator):
            ts = torch.full((b,), step, device=device, dtype=torch.long)
            index = torch.full_like(ts, fill_value=total_steps - i - 1)
            if i == 0 and use_lre:
                img = self.q_sample(x_start=lre, t=index)
            img = self.p_sample(
        ...
        # apply color correction (borrowed from StableSR)
        if color_fix_type == "adain":
            img_pixel = adaptive_instance_normalization(img_pixel, (cond_img + 1) / 2)
        elif color_fix_type == "wavelet":
            img_pixel = wavelet_reconstruction(img_pixel, (cond_img + 1) / 2)
        else:
            assert color_fix_type == "none", f"unexpected color fix type: {color_fix_type}"
        return img_pixel
